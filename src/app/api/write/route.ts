...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!userId && !req.nextUrl.pathname.startsWith('/api/write')) {
    ...
    switch (provider) {
      case 'openai':
        const openaiCompletion = await openai.chat.completions.create({
          ...
          messages: [
            {
              role: 'system',
              content: 'You are NebulaWrite, an AI writing assistant for AlienNova.com. You excel at creating high-quality written content based on user prompts. Your writing is engaging, well-structured, and tailored to the specified audience and purpose.'
            },
            {
              role: 'user',
              content: prompt
      ...
      case 'mistral':
        ...
        content = "Mistral API integration would be implemented here. For now, this is a placeholder response.";
        break;

      case 'cohere':
        // Fallback to OpenAI for now - would implement Cohere API in production
        content = "Cohere API integration would be implemented here. For now, this is a placeholder response.";
        break;

      case 'replicate':
        // Fallback to OpenAI for now - would implement Replicate API in production
        content = "Replicate API integration for Llama models would be implemented here. For now, this is a placeholder response.";
        break;
      ...
      default:
        ...
        const defaultCompletion = await openai.chat.completions.create({
          ...
          messages: [
            {
              role: 'system',
              content: 'You are NebulaWrite, an AI writing assistant for AlienNova.com.'
            },
            {
              role: 'user',
              content: prompt
        ...
        content = defaultCompletion.choices[0].message.content || '';
    }
